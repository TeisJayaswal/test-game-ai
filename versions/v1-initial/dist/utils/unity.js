 ...
 */
export function isValidUnityProject(projectPath) {
 ...
 */
export async function createUnityProjectStructure(projectPath, options = {}) {
    ...
    fs.writeFileSync(path.join(projectPath, 'Packages', 'manifest.json'), JSON.stringify(manifest, null, 2));
    // Create ProjectSettings/ProjectVersion.txt
    fs.writeFileSync(path.join(projectPath, 'ProjectSettings', 'ProjectVersion.txt'), 'm_EditorVersion: 2022.3.0f1\nm_EditorVersionWithRevision: 2022.3.0f1 (1234567890ab)');
    // Create ProjectSettings/ProjectSettings.asset (minimal)
    fs.writeFileSync(path.join(projectPath, 'ProjectSettings', 'ProjectSettings.asset'), createProjectSettingsAsset(path.basename(projectPath)));
}
...
function createProjectSettingsAsset(projectName) {
    return `%YAML 1.1
